...
import { speak, initTTS } from './services/speechService';
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-danger flex flex-col items-center justify-center p-6 text-white animate-fade-in overflow-hidden select-none">
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-md w-full">
         ...
         <h1 className="text-6xl font-black mb-12 tracking-tighter uppercase italic">SOS TRIGGERED</h1>
...
const ControlBar: React.FC<{ isConnected: boolean; isConnecting: boolean; isSpeaking: boolean; isListening: boolean; onConnect: () => void; onDisconnect: () => void; onToggleSpeech: () => void; onToggleListening: () => void; }> = ({ isConnected, isConnecting, isSpeaking, isListening, onConnect, onDisconnect, onToggleSpeech, onToggleListening }) => (
  <div className="flex-none p-4 lg:p-6 bg-surface border-t border-surfaceHighlight flex items-center justify-between gap-3 pb-[calc(1rem+env(safe-area-inset-bottom))] z-20 select-none transition-all">
    <div className="flex gap-2">
      <button onClick={() => { initTTS(); onToggleSpeech(); }} className={`p-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 border ${isSpeaking ? 'bg-surfaceHighlight/50 border-surfaceHighlight text-textMain hover:bg-surfaceHighlight' : 'bg-transparent border-surfaceHighlight/50 text-textMuted hover:text-textMain'}`}>{isSpeaking ? <SpeakerOnIcon className="w-6 h-6" /> : <SpeakerOffIcon className="w-6 h-6" />}</button>
...
const App: React.FC = () => {
  ...
  const handleConnectClick = () => {
    ...
    // Specific handling if Web Serial is missing (common in Android Wrappers/WebViews)
    if (!isSupported) {
      // If we are already connected remotely (Wizard mode), we might want to allow Fake Picker
      if (connectionStatus === 'connected') {
         setShowFakePicker(true);
         return;
      }
      
      // Otherwise, warn the user
      alert("⚠️ USB Not Supported in this App Wrapper\n\nAndroid Apps created by websites often use a 'WebView' that blocks USB connections.\n\nTo use your Arduino Nano:\n1. Open your app URL in Google Chrome for Android.\n2. Tap the menu (⋮) -> 'Install App' or 'Add to Home Screen'.\n3. Open the installed Chrome App to connect.");
      return;
    }
  ...
  const onFakeConnectConfirm = () => { 
    initTTS(); // Prime TTS engine
    setShowFakePicker(false); 
    setTimeout(() => setIsFakeConnected(true), 500); 
  };
    ...
    <div className="h-[100dvh] w-full flex flex-col bg-background text-textMain overflow-hidden font-sans select-none">
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        <div className="flex-1 lg:w-[400px] lg:flex-none flex flex-col bg-surface/20 lg:bg-surface/10 backdrop-blur-sm z-10 min-h-0">
          ...
          <ControlBar isConnected={effectiveIsConnected} isConnecting={isConnecting} isSpeaking={isSpeakingEnabled} isListening={isListening} onConnect={handleConnectClick} onDisconnect={handleDisconnectClick} onToggleSpeech={() => { initTTS(); setIsSpeakingEnabled(!isSpeakingEnabled); }} onToggleListening={toggleListening} />
        </div>
      </main>
    </div>
  );
